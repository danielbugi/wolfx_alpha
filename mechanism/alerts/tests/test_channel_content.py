...
def test_the_cards_render_valid_pngs_of_the_expected_width():
    for png in (cc.render_health_card(_health_card()),
        ...
        assert im.format == "PNG" and im.width == 1080 and 500 < im.height <= 1400
...
ALL = ["health", "sector", "macro", "gaps", "near_highs", "aligned", "base_rate", "recap", "promo"]
...
@pytest.mark.parametrize("kind", ALL)
def test_every_post_has_no_advice_words_no_claims_and_the_disclaimer(kind):
    plain = re.sub(r"<[^>]+>", "", cx.build_post(kind, _ctx()).text)
    assert BANNED.findall(plain) == [], BANNED.findall(plain)
    assert re.search(r"not investment advice", plain, re.I)
...
def test_every_education_and_promotion_text_is_clean_and_says_what_access_is(promo):
    ...
    if promo["assistant"]:                                                            # a post that promotes the assistant says exactly what access is today
        assert "beta" in cx.PRIVATE.lower() and "invitation" in cx.PRIVATE.lower()
...
def test_the_index_versus_breadth_sentence_only_appears_when_they_disagree():
    up_idx_weak = cx._index_vs_breadth(0.5, 980, 1894, 2915)
    assert up_idx_weak.startswith("Index up, most stocks down")
    assert cx._index_vs_breadth(-0.5, 2000, 800, 2915).startswith("Index down, most stocks up")
    assert cx._index_vs_breadth(0.5, 1800, 1000, 2915).startswith("S&amp;P 500")             # agree: a plain statement
    assert cx._index_vs_breadth(None, 1, 1, 10) is None and cx._index_vs_breadth(0.5, 0, 0, 10) is None


def test_health_post_states_the_before_numbers_only_when_they_exist():
    text = cx.post_health(_ctx()).text
    assert re.search(r"\(\d+% 10 sessions ago\)", text)
    short = _ctx()
...
def test_the_aligned_post_never_names_stocks_and_offers_the_assistant_button():
    ...
    assert p.button and "finviz" not in p.text and "Members of the private assistant" in p.text


def test_the_recap_lists_days_sectors_and_persistent_names():
    text = cx.post_recap(_ctx()).text
    assert "Breakouts by day: Mon 50" in text and "strongest Tech" in text and "GEMI" in text and "3+ of 5 sessions" in text
...
class FakeTG:
    ...
    def send_photo(self, png, caption="", silent=False, reply_markup=None):
        self.photos.append((caption, silent, reply_markup))

    def send_message(self, text, disable_preview=True, silent=False, reply_markup=None):
...
def test_extra_posts_are_silent_and_only_assistant_posts_carry_the_button():
    ...
    assert kb["url"].startswith("https://t.me/some_bot?start=ch") and kb["text"] == "Private assistant (invite only)"
...
def test_the_news_post_is_valid_html_escapes_third_party_text_and_shows_headline_link_and_source_only():
    ...
    assert re.search(r"not investment advice", post.text, re.I)
...
def test_the_scoreboard_post_states_both_sides_never_shows_negative_zero_and_carries_the_overlap_caveat():
    p = cx.post_scoreboard(_scoreboard_ctx())
    plain = re.sub(r"<[^>]+>", "", p.text)
    assert tg_html.problems(p.text) == [] and BANNED.findall(plain) == [] and re.search(r"not investment advice", plain, re.I)
    ...
    assert "not independent tests" in plain and "say nothing about the next ones" in plain and "60 sessions of lists, 25 Jun – 18 Sep" in plain
