#!/usr/bin/env python3
"""
Simple Donchian Screener - NO FILTERS
Pure breakout detection only - no volume, quality, or market cap filtering
"""
...
try:
    ...
    logger = setup_logging("simple_donchian")
...
class SimpleDonchianScreener:
    """Simple Donchian breakout screener with NO filtering"""

    def __init__(self):
        logger.info("Simple Donchian Screener - NO FILTERS MODE")
        logger.info("Detecting breakouts and near breakouts only")
    ...
    def check_donchian_breakout(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if breakout_type:
                ...
                atr = safe_float(latest['atr_14']) if latest['atr_14'] else current_price * 0.02
                ...
                return {
                    ...
                    'rsi_14': round(safe_float(latest['rsi_14']), 1) if latest['rsi_14'] else None,
                    'atr_14': round(safe_float(latest['atr_14']), 2) if latest['atr_14'] else None,
                    'sma_20': round(safe_float(latest['sma_20']), 2) if latest['sma_20'] else None,
                    'sma_50': round(safe_float(latest['sma_50']), 2) if latest['sma_50'] else None,
                    ...
                    'growth_score': fundamentals.get('growth_score'),
                    ...
                    'overall_quality_score': fundamentals.get('overall_quality_score'),
    ...
    def get_fundamentals(self, symbol: str) -> Dict:
        ...
        try:
            query = """
           SELECT market_cap, sector, industry, pe_ratio, pb_ratio, beta, quality_grade,
                growth_score, profitability_score, financial_health_score, valuation_score, overall_quality_score
    ...
    def save_results(self, results: List[Dict]) -> bool:
        ...
        try:
            ...
            logger.info(f"Simple results saved to {filename}")
  ...
  Near Bearish: {near_bearish}
        ...
        if results:
            ...
            for result in results[:10]:
                ...
                logger.info(f"    Urgency: {urgency}, Volume: {volume_ratio:.1f}x, Sector: {sector}")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Simple Donchian Screener - No Filters')
    parser.add_argument('--test', nargs='+', help='Test with specific symbols')
    args = parser.parse_args()

    screener = SimpleDonchianScreener()
