    ...
    def update_symbol(self, symbol: str) -> bool:
        ...
        try:
            ...
            if extended_data is not None and not extended_data.empty:
                ...
                # Update ALL recent technical indicator data (need historical context)
                # Filter to only recent data to avoid updating too much history
                recent_cutoff = (datetime.now() - timedelta(days=60)).date()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Daily Data Updater with Fixed RSI')
